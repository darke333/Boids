...
public class BoidSystemECSJobs : JobComponentSystem
{
    ...
    protected override JobHandle OnUpdate(JobHandle inputDeps)
    {
        ...
        if (controller != null)
        {
            ...
            NativeArray<LocalToWorld> localToWorldArray =
                boidQuery.ToComponentDataArray<LocalToWorld>(Allocator.TempJob);

            // These arrays get deallocated after job completion
            NativeArray<EntityWithLocalToWorld> boidArray =
                new NativeArray<EntityWithLocalToWorld>(entityArray.Length, Allocator.TempJob);
