    ...
    protected override void OnUpdate()
    {
        ...
        if (_controller != null)
        {
            EntityQuery boidQuery =
                GetEntityQuery(ComponentType.ReadOnly<BoidECS>(), ComponentType.ReadOnly<LocalToWorld>());
            NativeArray<float4x4> newBoidPositions =
                new NativeArray<float4x4>(boidQuery.CalculateEntityCount(), Allocator.Temp);
            ...
            Entities.WithAll<BoidECS>().ForEach((Entity boid, ref LocalToWorld localToWorld) =>
            {
                ...
                Entities.WithAll<BoidECS>().ForEach((Entity otherBoid, ref LocalToWorld otherLocalToWorld) =>
                {
                    if (boid != otherBoid)
                    {
                        ...
                        {
                            seperationSum += -(otherLocalToWorld.Position - boidPosition) *
                                             (1f / math.max(distToOtherBoid, .0001f));
